...
class Downloader:
    ...
    def __call__(self, url):
        ...
        if self.cache:
            try:
                result = self.cache[url]  # def __getitem__(self, url):
            except KeyError:
        ...
        if result is None:
            ...
            if self.cache:
                ...
                self.cache[url] = result  # def __setitem__(self, url, result):
        return result['html']
    ...
    def download(self, url, headers, proxy, num_retries, data=None):
        ...
        if proxy:
            proxy_params = {tullib.parse.urlparse(url).scheme: proxy}
