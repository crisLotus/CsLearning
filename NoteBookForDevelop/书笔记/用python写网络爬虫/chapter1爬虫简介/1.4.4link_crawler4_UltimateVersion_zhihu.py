...
def link_crawler(seed_url, link_regex=None, delay=5, max_depth=-1, max_urls=-1, headers=None, user_agent='wswp', proxy=None, num_retries=1):
    ...
    while crawl_queue:
        ...
        if rp.can_fetch(user_agent, url):
            ...
            throttle.wait(url)
            html = download(url, headers, proxy=proxy,
                            num_retries=num_retries).decode("utf8")
            links = []
...
def normalize(seed_url, link):
    ...
    """
    link, _ = urllib.parse.urldefrag(
        link)  # remove hash to avoid duplicates 把url #后的部分赋给量变 _
    return urllib.parse.urljoin(seed_url, link)  # 连接url协议域名部分和虚拟目录部分
...
if __name__ == '__main__':
    ...
    link_crawler('https://www.zhihu.com/', '/', delay=2,
                 num_retries=1, max_depth=1, user_agent=user_agent)
